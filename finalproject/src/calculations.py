...
class LCACalculator:
    ...
    def compare_alternatives(self, impacts: pd.DataFrame, product_ids: List[str]) -> pd.DataFrame:
        """
        Compare environmental impacts of selected products by aggregating and normalizing impact scores.
    
        Args:
            impacts: DataFrame with calculated impacts for all product stages.
            product_ids: List of product IDs to compare.
    
        Returns:
            DataFrame with total and relative impacts for each selected product.
        """
        # Filter only the selected products
        filtered = impacts[impacts["product_id"].isin(product_ids)]
    
        if filtered.empty:
            raise ValueError("No matching product IDs found in the impacts data.")
    
        # Aggregate total impacts per product
        comparison = filtered.groupby(["product_id", "product_name"], as_index=False).agg({
            "carbon_impact": "sum",
            "energy_impact": "sum",
            "water_impact": "sum",
            "waste_generated_kg": "sum"
        ...
        # Normalize (relative to max of each column)
        for col in ["carbon_impact", "energy_impact", "water_impact", "waste_generated_kg"]:
            max_val = comparison[col].max()
            relative_col = f"{col}_relative"
            comparison[relative_col] = comparison[col] / max_val if max_val != 0 else 0
